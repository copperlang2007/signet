...
export function validateContract(c) {
  ...
  // Every acceptance-criterion phase must be a budgeted phase (RED_TEAM round 5): a criterion
  // in an unbudgeted phase names work no cap governs. Cross-check once both lists are present.
  if (Array.isArray(c.acceptance_criteria) && c.budget && Array.isArray(c.budget.phases)) {
    const budgeted = new Set(c.budget.phases.map((ph) => ph.name));
    for (const a of c.acceptance_criteria) {
      if (isStr(a.phase) && !budgeted.has(a.phase)) err(`acceptance criterion ${a.id ?? '?'} names phase "${a.phase}" with no matching budget.phases entry`);
    }
  }
  ...
  if (!Array.isArray(c.assumptions)) err('assumptions[] required');
